...
from massive_direct_stream import DirectMassiveStreamConfig, MassiveDirectDequeStreamer
...
from reversal_module import QuoteReversalConfig
...
# Debug / diagnostics
# =========================
ENABLE_SYMBOL_STATE_DEBUG_PRINTING = False
...
ENABLE_SYMBOL_STATE_DEBUG_PRINT_TO_CONSOLE = False
ENABLE_SYMBOL_STATE_DEBUG_WRITE_TO_FILE = False
...
# Mongo reversal baseline exit/reverse.
# Detection thresholds are built from quote_reversal_symbol_summary documents in Mongo.
# No static tick-quote reversal thresholds are used here.
ENABLE_MONGO_QUOTE_REVERSAL_EXIT = True

# REVERSE = use Mongo reverse_thresholds first, then Mongo close_thresholds as fallback.
# CLOSE   = use Mongo close_thresholds only.
MONGO_QUOTE_REVERSAL_ACTION_MODE = "REVERSE"

MONGO_QUOTE_REVERSAL_CLOSE_THRESHOLD_GROUP = "close_thresholds"
MONGO_QUOTE_REVERSAL_REVERSE_THRESHOLD_GROUP = "reverse_thresholds"
...
# =========================
# Raw short-key market event helpers
# =========================
...
mongo_quote_reversal_config_warning_keys: set[tuple[str, str, str, str]] = set()
...
def required_non_negative_float(
    source: dict[str, Any],
    key: str,
    *,
    label: str,
...
def build_mongo_quote_reversal_config(
    *,
    baseline: ReversalBaselineSnapshot,
    reversal_side: str,
    threshold_group: str,
    allow_reverse: bool,
) -> QuoteReversalConfig:
    side_thresholds = baseline.thresholds.get(reversal_side) or {}
    ...
    settings = baseline.settings or {}

    return QuoteReversalConfig(
        enabled=ENABLE_MONGO_QUOTE_REVERSAL_EXIT,
        lookback_ms=required_positive_int(
            settings,
            "lookback_ms",
            label=f"settings.lookback_ms for {baseline.symbol}",
        ),
        ...
        minimum_persist_ms=required_positive_int(
            thresholds,
            "minimum_persist_ms",
...
def quote_reversal_config_to_dict(config: QuoteReversalConfig) -> dict[str, Any]:
    return {
        "enabled": getattr(config, "enabled", None),
        "lookback_ms": getattr(config, "lookback_ms", None),
        "minimum_ticks": getattr(config, "minimum_ticks", None),
        "minimum_persist_ms": getattr(config, "minimum_persist_ms", None),
        "minimum_mid_move": getattr(config, "minimum_mid_move", None),
        "minimum_mid_move_percent": getattr(config, "minimum_mid_move_percent", None),
        "maximum_spread_percent": getattr(config, "maximum_spread_percent", None),
        "require_bid_ask_mid_same_direction": getattr(
            config,
            "require_bid_ask_mid_same_direction",
            None,
        ),
        "allow_reverse": getattr(config, "allow_reverse", None),
    }


def maybe_build_mongo_quote_reversal_config(
    *,
    symbol: str,
    baseline: ReversalBaselineSnapshot,
    reversal_side: str,
    threshold_group: str,
    allow_reverse: bool,
) -> QuoteReversalConfig | None:
    try:
        return build_mongo_quote_reversal_config(
            baseline=baseline,
        ...
        warning_key = (symbol, baseline.document_id, threshold_group, str(exc))
        if warning_key not in mongo_quote_reversal_config_warning_keys:
            mongo_quote_reversal_config_warning_keys.add(warning_key)
            logger.warning(
                "Skipping Mongo quote reversal config for %s: %s",
                symbol,
                exc,
            )
        return None
...
# This file now consumes raw short-key Massive events directly; no Massive stream
# reader, source ids, or field wrapper are used.
...
async def main() -> None:
    ...
    async def on_trigger_candidates(
        candidates: list[TriggerDecision],
        payload: dict[str, Any],
    ) -> None:
        # Trigger candidate printing remains enabled inside print_trigger_candidate_loop.
        return None
    ...
    trigger_candidate_task = asyncio.create_task(
        print_trigger_candidate_loop(
            ...
            on_candidates=on_trigger_candidates,
        )
    )
